...
```{r exercise 01-2}
diamonds_prc <- diamonds[,c("price","cut","color")]
```
...
**Answer 1:** false false true false false
```{r exercise 05-1}
# it's a sequence from -4 to 4 with interval of 2, it shows as -4, -2, 0, 2, 4)
x <- seq(-4, 4, by = 2)
(x == 0)
```
**Answer 2:**true true false false false 
```{r exercise 05-2}
x <- seq(-4, 4, 2)
y <- 1:5
(x + y <= 0)
...
**Answer 3:**false false true true true 
```{r exercise 05-3}
#-4, 0, 4, 8, 12
x <- seq(-4, 12, 4)
y <- 1:5
(x > y)
```
**Answer 4:**false false true true false
```{r exercise 05-4}
x <- seq(-4, 12, 4)
y <- 1:5
(x > y) & ((x < 10) | (y < 3))
...
```{r exercise 10-2}
msleep %>%
  mutate(is_big = bodywt > 50,.after = name)
...
```{r exercise 10-4}
msleep %>%
  mutate(Z_weight = (bodywt - mean(bodywt))/sd(bodywt))
```
