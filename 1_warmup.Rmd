...
population_vector <- c(minors, adults, seniors)
```
...
```{r}
population <- tibble(age = population_vector)
mean(population$age)
min(population)
max(population)
```
...
```{r}
population_greater_than_50 <- population %>%
  filter(age > 50)

mean(population_greater_than_50$age)
```

4.  How many people have a round age (i.e., an age that is divisible by 10)? What is their mean age?

```{r}
population_round_age <- population %>%
  filter(age%%10 == 0)
mean(population_round_age$age)
```
...
ggplot(mpg, aes(x = cty)) +    # set mappings for ALL geoms
  ...
  geom_freqpoly(color = "steelblue", size = 2, aes(x = cty)) +
